...
void RTG::Configuration::usage(std::function< void(const char *, const char *) > const &callback) {
	...
	callback("--camera <camera name>", "View the scene through camera with name <camera>.");
...
RTG::RTG(Configuration const &configuration_) : helpers(*this) {
	...
	
	{
		...
		//add extenesion for moltenVK portability layer on macOS
	#if defined(__APPLE__)
		...
		// write debug messenger structuire
...
void RTG::recreate_swapchain() {
	//cleanup swapchian if it already exist
	...
	if (configuration.headless) {
		...
		// determin siz,e image count, and transform for swapchin
